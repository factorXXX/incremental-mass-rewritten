	...
	for (let x = 0; x < RANKS.names.length; x++) {
		...
		table += `<div style="width: 300px" id="ranks_div_${x}">
			...
			<button onclick="RANKS.reset('${rn}')" class="btn reset" id="ranks_${x}">
				Reset your ${x>0?RANKS.fullNames[x-1]+"s":'mass, upgrades and magic feature'}, but ${RANKS.fullNames[x]} up.<span id="ranks_desc_${x}"></span><br>
...
function updateMagicHTML() {
	...
	for(let i=0; i<=2; i++){
		...
		tmp.el["god"+i].setDisplay(hasUpgrade("ma",3))
		tmp.el["godon"+i].setTxt(player.ma.god==i?"GOD: ON":"GOD: OFF")
		tmp.el["god"+i].setClasses({btn: true, locked: !(player.ma.god==i||player.ma.god==-1)})
	}
}
